        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="title mb-30">
                    <h2>{{ isset($viewMode) && $viewMode ? 'Detail Role' : 'Ubah Role' }}</h2>
                </div>
            </div>
            <div class="col-md-6 text-end">
                <a href="{{ route('user-roles.index') }}" class="btn btn-secondary">Kembali</a>
            </div>
        </div>
    </div>
        ...
        <div class="card-style-3 mb-30">
            <div class="card-content">
                @if(isset($viewMode) && !$viewMode)
                    <form id="editForm" action="{{ route('user-roles.update', $userRole) }}" method="POST">
                        @csrf
                        @method('PUT')
                        @endif
